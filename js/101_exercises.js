...
function isMultipleOfBothThreeAndFive (M35) {
    if (M35 % 3 === 0 && M35 % 5 === 0) {
        return (true);
    }
    if (M35 % 3 !== 0 || M35 % 5 !== 0) {
...
function quotient (quo, off) {
    return (Math.floor (quo / off));
}
...
function sumOfSquares (sum1, sum2) {
...
function circumference (sir) {
    return ((sir * 2) * Math.PI);
}
...
function isVowel (vwl){
    return vwl == "a"|| vwl == "A" || vwl =="e"|| vwl =="E"|| vwl =="i"|| vwl =="I"|| vwl =="o"|| vwl =="O"|| vwl =="U"|| vwl =="u"
    return (true);
}
...
function hasVowels (vwl){
    return (vwl.includes ("a")|| vwl.includes ("e")||vwl.includes ("i")||vwl.includes ("o")||vwl.includes ("u"));
}
...
function countVowels (cv) {
    var vowelsCount = 0;
    //turn the input into a string
    var string = cv.toString();
...
// Exercise 53
// Write a function definition named fourth that takes in sequence and returns the forth value of that sequence.
...
function firstAndSecond(fas){
    return [first(fas), second(fas)]
}
...
function firstAndLast(fal){
    return [first(fal), last(fal)];
}
...
//need to loop through array and add values
function sumAll(sumA){
    var counter = 0;
...
assert(sumAll([3, 3, 3,0]), 9, "Exercise 60");
assert(sumAll([0, 5, 6,0]), 11, "Exercise 60");
...
function mean(array){
    var addition = sumAll(array)
    var variables = (array.length)
    return (addition / variables)
}
...
function maxMinusMin(Mn){
    var order = Mn.sort();
    //console.log(order)
    var first = Mn[0];
    var last = Mn[Mn.length -1];
...
//changed counter from 0 to 1 to get starting point thats not 0.
